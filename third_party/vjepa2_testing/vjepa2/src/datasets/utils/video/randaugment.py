...
def _rotate_level_to_arg(level, hparams):
    # range [-max_deg, max_deg]
    max_degrees = hparams.get("max_rotate_degrees", 30.0)
    level = (level / _MAX_LEVEL) * float(max_degrees)
...
def _enhance_increasing_level_to_arg(level, hparams):
    ...
    # range [0.1, 1.9]
    max_delta = float(hparams.get("max_enhance_increase", 0.9))
    level = (level / _MAX_LEVEL) * max_delta
    level = 1.0 + _randomly_negate(level)
    return (level,)


def _shear_level_to_arg(level, hparams):
    # range [-max_shear, max_shear]
    max_shear = float(hparams.get("max_shear", 0.3))
    level = (level / _MAX_LEVEL) * max_shear
    level = _randomly_negate(level)
    return (level,)
...
def _solarize_add_level_to_arg(level, hparams):
    # range [0, max_value]
    max_value = int(hparams.get("solarize_add_max", 110))
    return (int((level / _MAX_LEVEL) * max_value),)
...
def _apply_op_overrides(ops, overrides):
    for op in ops:
        cfg = overrides.get(op.name)
        if not cfg:
            continue
        ...
        for key, value in cfg.items():
            if key in {"enabled", "prob"}:
                continue
            op.hparams[key] = value


def rand_augment_transform(config_str, hparams, disabled_ops=None, op_overrides=None):
    """
    ...
    disabled_set = set(disabled_ops or [])
    if op_overrides:
        disabled_set.update({name for name, cfg in op_overrides.items() if cfg.get("enabled") is False})
    if disabled_set:
        transforms = [name for name in transforms if name not in disabled_set]
    if not transforms:
        raise ValueError("RandAugment configuration removed all available transforms.")
    ra_ops = rand_augment_ops(magnitude=magnitude, hparams=hparams, transforms=transforms)
    if op_overrides:
        _apply_op_overrides(ra_ops, op_overrides)
    choice_weights = None if weight_idx is None else _select_rand_weights(weight_idx, transforms=transforms)
