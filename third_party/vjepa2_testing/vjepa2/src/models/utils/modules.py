...
# Backwards/forwards compatible SDPA context: prefer the new torch.nn.attention.sdpa_kernel
# and fall back to the deprecated torch.backends.cuda.sdp_kernel on older Torch.
try:  # torch>=2.3
    from torch.nn.attention import sdpa_kernel as _sdpa_kernel_ctx  # type: ignore
    try:
        # Torch 2.3+ exposes SDPBackend here; older versions only under torch.backends.cuda
        from torch.nn.attention import SDPBackend as _SDPBackend  # type: ignore
    except Exception:  # pragma: no cover
        from torch.backends.cuda import SDPBackend as _SDPBackend  # type: ignore
except Exception:  # pragma: no cover
    _sdpa_kernel_ctx = None
    _SDPBackend = None


_SDPA_POLICY_ENV = "INFERENCE_VJEPA_SDPA_POLICY"
_SDPA_POLICY_AUTO = "auto"
_SDPA_POLICY_NO_FLASH = "no_flash"
_SDPA_POLICY_MATH = "math"
_SDPA_POLICIES = {_SDPA_POLICY_AUTO, _SDPA_POLICY_NO_FLASH, _SDPA_POLICY_MATH}


@functools.lru_cache(maxsize=1)
def _resolve_sdpa_policy() -> str:
    """Resolve the SDPA backend policy.

    This is treated as static for the lifetime of the process to avoid overhead in hot paths.
    """
    raw = (os.getenv(_SDPA_POLICY_ENV, _SDPA_POLICY_AUTO) or _SDPA_POLICY_AUTO).strip().lower()
    if raw in _SDPA_POLICIES:
        return raw
    return _SDPA_POLICY_AUTO


@functools.lru_cache(maxsize=None)
def _sdpa_backends_for_policy(policy: str):
    if _SDPBackend is None:
        return None
    policy = policy if policy in _SDPA_POLICIES else _SDPA_POLICY_AUTO
    if policy == _SDPA_POLICY_MATH:
        backends = [getattr(_SDPBackend, "MATH", None)]
    elif policy == _SDPA_POLICY_NO_FLASH:
        backends = [
            getattr(_SDPBackend, "EFFICIENT_ATTENTION", None),
            getattr(_SDPBackend, "MATH", None),
        ]
    else:
        backends = [
            getattr(_SDPBackend, "FLASH_ATTENTION", None),
            getattr(_SDPBackend, "EFFICIENT_ATTENTION", None),
            getattr(_SDPBackend, "MATH", None),
        ]
    return [b for b in backends if b is not None] or None


def _sdpa_legacy_flags(policy: str) -> tuple[bool, bool, bool]:
    policy = policy if policy in _SDPA_POLICIES else _SDPA_POLICY_AUTO
    enable_flash = policy == _SDPA_POLICY_AUTO
    enable_mem_efficient = policy in {_SDPA_POLICY_AUTO, _SDPA_POLICY_NO_FLASH}
    enable_math = True
    return enable_flash, enable_mem_efficient, enable_math


@contextmanager
def _sdpa_ctx():
    policy = _resolve_sdpa_policy()
    enable_flash, enable_mem_efficient, enable_math = _sdpa_legacy_flags(policy)
    if _sdpa_kernel_ctx is None:
        sdp_kernel_ctx = getattr(torch.backends.cuda, "sdp_kernel", None)
        if sdp_kernel_ctx is None:
            yield
            return
        try:
            with sdp_kernel_ctx(
            ...
            # Some older torch versions require positional arguments.
            with sdp_kernel_ctx(enable_flash, enable_math, enable_mem_efficient):
                yield
                return

    # New API: torch.nn.attention.sdpa_kernel may require explicit arguments depending on Torch version.
    backend_list = _sdpa_backends_for_policy(policy)
    for ctx_kwargs in (
        {"backends": backend_list} if backend_list else None,
        {"enable_flash": enable_flash, "enable_math": enable_math, "enable_mem_efficient": enable_mem_efficient},
        {} if policy == _SDPA_POLICY_AUTO else None,  # no-arg call is only safe when flash is allowed
    ):
        if ctx_kwargs is None:
            continue
        try:
            with _sdpa_kernel_ctx(**ctx_kwargs):
                yield
                return
        except Exception:
            continue

    # Last resort: prefer explicit legacy backend selection (avoid implicit defaults).
    sdp_kernel_ctx = getattr(torch.backends.cuda, "sdp_kernel", None)
    ...
    # Final fallback: do not crash; leave backend selection to torch defaults.
    yield
...
def rotate_queries_or_keys(x, pos):
    ...
    # Keep RoPE position tensor dtype/device aligned with q/k to avoid ONNX
    # export generating mixed-type einsum inputs (int64/float).
