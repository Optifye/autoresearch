...
def _warp_preproc_enabled() -> bool:
    raw = os.getenv("INFERENCE_VJEPA_WARP_PREPROC")
    if raw is None:
        raw = os.getenv("VJEPA_WARP_PREPROC")
...
def make_transforms(
    ...
    autoaugment_disabled_ops=None,
    randaugment_overrides=None,
):

    warp_preproc = _warp_preproc_enabled()
    ...
    if not training and num_views_per_clip > 1:
        ...
        _frames_augmentation = VideoTransform(
            ...
            crop_size=crop_size,
            normalize=normalize,
            autoaugment_disabled_ops=autoaugment_disabled_ops,
            randaugment_overrides=randaugment_overrides,
            warp_preproc=warp_preproc,
        )
...
class VideoTransform(object):
    ...
    def __init__(
        ...
        self.warp_preproc = bool(warp_preproc)

        if self.warp_preproc:
        ...
        self.autoaug_transform = video_transforms.create_random_augment(
            ...
            disabled_ops=autoaugment_disabled_ops,
            op_overrides=randaugment_overrides,
        )
...
class EvalVideoTransform(object):
    ...
    def __init__(
        ...
        self.warp_preproc = bool(warp_preproc)
        if self.warp_preproc:
            self.spatial_resize = video_transforms.Resize((short_side_size, short_side_size), interpolation="bilinear")
        else:
