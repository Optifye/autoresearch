...
def load_checkpoint_weights(
    ...
    *,
    strict=False,
    load_predictor=True,
    load_target_encoder=True,
):
    """
    Initialize model weights from a checkpoint WITHOUT restoring optimizer/scaler/epoch.

    This is intended for "continue pretraining" / domain-adaptive pretraining where we want to:
    - start a fresh optimization schedule (epoch=0, new optimizer state)
    - optionally relax strict loading to tolerate mask-token count differences
    """
    logger.info(
        "Initializing weights from checkpoint %s (strict=%s load_predictor=%s load_target_encoder=%s)",
        r_path,
        strict,
        load_predictor,
        load_target_encoder,
    ...
    # -- loading encoder (fallback to target_encoder if encoder key missing)
    if "encoder" in checkpoint:
        pretrained_dict = checkpoint["encoder"]
    elif "target_encoder" in checkpoint:
        pretrained_dict = checkpoint["target_encoder"]
        logger.warning('Checkpoint missing "encoder"; falling back to "target_encoder" weights.')
    else:
        raise KeyError('Checkpoint missing required key "encoder" (and no "target_encoder" fallback).')
    msg = encoder.load_state_dict(pretrained_dict, strict=strict)
    logger.info(f"initialized encoder weights with msg: {msg}")

    # -- loading predictor (optional)
    if predictor is not None and load_predictor:
        if "predictor" not in checkpoint:
            logger.warning('Checkpoint missing "predictor"; leaving predictor randomly initialized.')
        else:
            pretrained_dict = checkpoint["predictor"]
            msg = predictor.load_state_dict(pretrained_dict, strict=strict)
            logger.info(f"initialized predictor weights with msg: {msg}")

    # -- loading target_encoder (optional)
    if target_encoder is not None and load_target_encoder:
        if "target_encoder" not in checkpoint:
            logger.warning('Checkpoint missing "target_encoder"; leaving target encoder as deepcopy(encoder).')
        else:
            pretrained_dict = checkpoint["target_encoder"]
            msg = target_encoder.load_state_dict(pretrained_dict, strict=strict)
            logger.info(f"initialized target encoder weights with msg: {msg}")
