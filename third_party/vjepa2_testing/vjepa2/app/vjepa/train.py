...
from app.vjepa.utils import init_opt, init_video_model, load_checkpoint, load_checkpoint_weights
...
def main(args, resume_preempt=False):
    ...
    init_from_ckpt = cfgs_meta.get("init_from_checkpoint", None)
    init_strict = cfgs_meta.get("init_checkpoint_strict", False)
    init_load_predictor = cfgs_meta.get("init_load_predictor", True)
    init_load_target_encoder = cfgs_meta.get("init_load_target_encoder", True)
    ...
    if load_model or os.path.exists(latest_path):
        ...
        if not is_anneal or resume_anneal:
            for _ in range(start_epoch * ipe):
                ...
                mask_collator.step()
    elif init_from_ckpt is not None and os.path.exists(init_from_ckpt):
        load_checkpoint_weights(
            r_path=init_from_ckpt,
            encoder=encoder,
            predictor=predictor,
            target_encoder=target_encoder,
            strict=bool(init_strict),
            load_predictor=bool(init_load_predictor),
            load_target_encoder=bool(init_load_target_encoder),
        )
        logger.info("Initialized weights from checkpoint %s; starting fresh optimization schedule (epoch=0).", init_from_ckpt)
